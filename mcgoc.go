...
func (accountBearer MCbearers) CreatePayloads(name string) Payload {
	...
	for _, bearer := range accountBearer.Bearers {
	...
	return Payload{Payload: payload, Conns: conns, UNIX: dropTime}
}
...
func Auth(accounts []string) (MCbearers, error) {
	...
	for _, info := range accounts {
		...
		if strings.Contains(string(respBytes), "Sign in to") {
			bearer = "Invalid"
			return MCbearers{nil}, errors.New("Invalid Credentials")
		}

		if strings.Contains(string(respBytes), "Help us protect your account") {
			bearer = "Invalid"
			return MCbearers{nil}, errors.New("Account has security questions!")
		}
		...
		if bearer != "Invalid" {
			...
			case 401:
				...
				case !strings.Contains(string(xsBody), "2148916238"):
					...
					return MCbearers{nil}, errors.New("account belongs to someone under 18 and needs to be added to a family")
				case !strings.Contains(string(xsBody), "2148916233"):

					return MCbearers{nil}, errors.New("account has no Xbox account, you must sign up for one first")
				}
			}
	...
	return MCbearers{Bearers: bearerReturn}, nil
}
