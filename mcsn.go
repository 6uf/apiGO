...
func authAcc(accounts []string) (MCbearers, error) {
	...
	cfg := yacspin.Config{
		...
		Suffix:          fmt.Sprintf("%v", aurora.Bold(aurora.White((" Initiation")))),
	...
	for _, info := range accounts {
		if i == 3 {
			spinner.Message(fmt.Sprintf("%v", aurora.Bold(aurora.White(("[LOG] Sleeping to avoid rate limit. (1 Minute)")))))
		...
		if strings.Contains(string(respBytes), "Help us protect your account") {
			bearer = "Invalid"
			spinner.Message(fmt.Sprintf("%v", aurora.Sprintf(aurora.Bold(aurora.White(("Account has security questions! | %v"))), aurora.Bold(aurora.Red(email)))))
		...
		if bearer != "Invalid" {
			...
			case 401:
				...
				case !strings.Contains(string(xsBody), "2148916238"):
					spinner.Message(fmt.Sprintf("%v", aurora.Sprintf(aurora.Bold(aurora.White(("account belongs to someone under 18 and needs to be added to a family | %v"))), aurora.Bold(aurora.Red(email)))))
					time.Sleep(1 * time.Second)
					continue
				case !strings.Contains(string(xsBody), "2148916233"):
					spinner.Message(fmt.Sprintf("%v", aurora.Sprintf(aurora.Bold(aurora.White(("account has no Xbox account, you must sign up for one first | %v"))), aurora.Bold(aurora.Red(email)))))
					time.Sleep(1 * time.Second)
					continue
				}
			}
			...
			if g == 10 {
				spinner.Message(fmt.Sprintf("%v", aurora.Sprintf(aurora.Bold(aurora.White(("Sleeping for %v seconds to avoid Mojang rate limit."))), aurora.Bold(aurora.Red("30")))))
			...
			if resp.StatusCode == 204 {
				spinner.Message(fmt.Sprintf("%v", aurora.Sprintf(aurora.Bold(aurora.White(("[MOJANG] Authenticated | %v"))), aurora.Bold(aurora.Red(email)))))
				bearerReturn = append(bearerReturn, *access.AccessToken)
				accountType = append(accountType, "Microsoft")
			} else {
				spinner.Message(fmt.Sprintf("%v", aurora.Sprintf(aurora.Bold(aurora.White(("[MCSN] Couldnt Auth | %v"))), aurora.Bold(aurora.Red(email)))))
			}
